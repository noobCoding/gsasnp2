...
public:
	...
	double monotoneCubicSplineInterp(double x)
	{
		int n = (int)xx.size();
		myPoint tmpPoint;
		tmpPoint.x = 0.000000001;
		tmpPoint.y = 0.000000001;
	...
	void Polynomial(vector<double> x, vector<double> y, int nDegree, vector<double> &esCoef) { // n is the degree of Polynomial 
		...
		for(int i = 0; i < n+1; i++) a[i]=0.;				//B is the Normal matrix(augmented) that will store the equations, 'a' is for value of the final coefficients
		double** B = new double*[nDegree + 1];
		for (int i = 0; i < nDegree + 1; i++) {
			B[i] = new double[nDegree + 2];  
			for(int j = 0; j < nDegree + 2; j++) B[i][j]=0.;
		...
		double* Y = new double[nDegree + 1];					//Array to store the values of sigma(yi),sigma(xi*yi),sigma(xi^2*yi)...sigma(xi^n*yi)
		for (int i = 0; i < n + 1; i++)
		{
			Y[i] = 0;
			for (int j = 0; j < Ndata; j++)
